...
export function NodeResourceBrowser({
  ...
  return (
    <div className="node-resource-browser">
      <div className="node-category-tabs" role="tablist" aria-label={zh ? "节点类型" : "Node types"}>
        {tabs.map((tab) => {
          const Icon = tab.icon;
          ...
          return (
            ...
            >
              <span className="node-category-tab-icon"><Icon size={16} /></span>
              <span>{tab.label}</span>
      ...
      <PageToolbar
        placeholder={zh ? "搜索节点名称、集群、IP 或任务名称..." : "Search node, cluster, IP or task..."}
        value={query}
        onChange={setQuery}
      ...
      <section className="panel node-resource-table-panel">
        <div className="node-resource-table-summary">
          <div>
            ...
            <small>
              {filteredNodes.length} {zh ? "个节点" : "nodes"} · {onlineCount} {zh ? "在线" : "online"}
            </small>
          </div>
          ...
          <div className="node-resource-table-body">
              ...
              return (
                ...
                >
                  <span className="node-row-primary">
                    ...
                    <strong className="node-row-name">{node.metadata.name}</strong>
                  </span>
                  <span className={`node-type-cell cat-${nodeCategory}`}>
                    <CategoryIcon size={14} />
                    {zh ? categoryInfo.zh : categoryInfo.en}
                  </span>
                  <span><StatusBadge phase={phase} copy={c} /></span>
                  <span className="node-row-meta" title={cluster}>{cluster}</span>
                  <code className="node-row-ip">{address}</code>
                  <span className={`embodied-task-state ${hasEmbodiedTask ? "active" : "idle"}`}>
                    {hasEmbodiedTask ? (zh ? "运行中" : "Running") : (zh ? "无" : "None")}
                  </span>
                  <span className="node-row-task" title={taskName}>{taskName || "—"}</span>
            ...
            {pagedNodes.length === 0 && (
              <div className="node-resource-empty">
                <LayoutGrid size={22} />
                <strong>{zh ? "没有符合条件的节点" : "No matching nodes"}</strong>
                <small>{zh ? "尝试切换分类或清除筛选条件" : "Try another category or filter"}</small>
              </div>
            )}
          </div>
        </div>
