...
提交前，检查生成的 YAML 清单。该 YAML 是 Kubernetes 兼容的 CRD（`Job.rlinf.io/v1alpha1`），描述了完整的任务规格。

确认后提交任务。你可以从任务列表监控任务状态，实时查看 Worker 的配置、初始化和执行情况。
...
从 Worker 列表中点击任意 Worker 的**终端**操作，将在 Worker 主容器中打开运行 `/bin/sh` 的 WebTerminal 会话。
