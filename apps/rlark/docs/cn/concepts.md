...
业务平台的节点总数和集群详情节点列表包含标注为 `rlark.io/node-category=cloud`、`edge` 或 `robot` 的可用 Worker；对于历史上未标注分类、但明确上报 GPU 或具身设备资源的节点，也作为 Worker 展示。带有 Kubernetes `master` 或 `control-plane` 角色标签的节点仍只在管理平台中展示，不作为业务任务 Worker 统计和展示。
