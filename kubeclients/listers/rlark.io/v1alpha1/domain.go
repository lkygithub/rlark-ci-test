...
type DomainLister interface {
	...
	List(selector labels.Selector) (ret []*rlarkiov1alpha1.Domain, err error)
	// Get retrieves the Domain from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*rlarkiov1alpha1.Domain, error)
	DomainListerExpansion
}

// domainLister implements the DomainLister interface.
type domainLister struct {
	listers.ResourceIndexer[*rlarkiov1alpha1.Domain]
}

// NewDomainLister returns a new DomainLister.
func NewDomainLister(indexer cache.Indexer) DomainLister {
	return &domainLister{listers.New[*rlarkiov1alpha1.Domain](indexer, rlarkiov1alpha1.Resource("domain"))}
}
